...
export default function AllHotDeals({navigation, route}) {
  ...
  const llHotDealsCarOwners = firestore().collectionGroup('CarOwner');

  async function getAllHotDeals() {
    const snapshot = await llHotDealsCarOwners
      .limit(10)
      .orderBy('rating', 'desc')
      .get()
      .catch(err => {
  ...
  async function getMore() {
    if (lastDoc) {
      ...
      setTimeout(async () => {
        let snapshot = await llHotDealsCarOwners
          .orderBy('rating', 'desc')
